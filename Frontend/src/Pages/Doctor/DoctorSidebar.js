import React, { useState } from "react";
import { Link } from "react-router-dom";
import {
  MdPersonSearch,
  MdHealthAndSafety,
  MdPeople,
  MdWarning,
  MdMedication,
} from "react-icons/md"; // Example icons
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faPlus } from "@fortawesome/free-solid-svg-icons";
const Sidebar = () => {
  const iconSize = "30px"; // Adjust icon size as needed
  const [activeMenu, setActiveMenu] = useState(null);

  const handleMenuClick = (menuName) => {
    setActiveMenu(menuName);
  };

  return (
    <div className="text-black h-full flex flex-col justify-center items-center">
      <ul className="space-y-8 mt-8">
        {" "}
        {/* Increased vertical space */}
        
        <li>
          <Link
            to="/doctoranalysis"
            className={`flex items-center space-x-2 hover:text-red-700 ${
              activeMenu === "analysis" ? "text-rose-500" : ""
            }`}
            onClick={() => handleMenuClick("analysis")}
          >
            <MdHealthAndSafety size={iconSize} />
            <span className="text-2xl"> Analysis</span>
          </Link>
        </li>
      
        <li>
          <Link
            to="/addconsultancy"
            className={`flex items-center space-x-2 hover:text-red-700 ${
              activeMenu === "addConsultancy" ? "text-rose-500" : ""
            }`}
            onClick={() => handleMenuClick("addConsultancy")}
          >
            <FontAwesomeIcon icon={faPlus} size={iconSize} />
            <span className="text-2xl">Add Consultancy</span>
          </Link>
        </li>
        <li>
          <Link
            to="/adddegree"
            className={`flex items-center space-x-2 hover:text-red-700 ${
              activeMenu === "addDegree" ? "text-rose-500" : ""
            }`}
            onClick={() => handleMenuClick("addDegree")}
          >
            <FontAwesomeIcon icon={faPlus} size={iconSize} />
            <span className="text-2xl">Add Degree</span>
          </Link>
        </li>

        
      </ul>
    </div>
  );
};

export default Sidebar;
